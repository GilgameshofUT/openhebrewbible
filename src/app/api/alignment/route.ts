...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!Array.isArray(alignment.words) || alignment.words.length === 0) {
      return NextResponse.json({ words: [] }, { headers: { 'Cache-Control': 'public, max-age=3600, s-maxage=31536000, stale-while-revalidate=86400' } })
    }
    return NextResponse.json({ words: alignment.words }, { headers: { 'Cache-Control': 'public, max-age=3600, s-maxage=31536000, stale-while-revalidate=86400' } })
  } catch {
